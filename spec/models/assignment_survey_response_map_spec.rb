describe AssignmentSurveyResponseMap, type: :model do
  let(:participant) { build(:participant, user_id: 1, user: build(:student, username: 'Jane', fullname: 'Doe, Jane', id: 1)) }
  ...
  let(:user) { build(:student, email: 'expertiza.debugging@gmail.com', nausernameme: 'Jane', fullname: 'Doe, Jane', id: 1) }
  before(:each) do
